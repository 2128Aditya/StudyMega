...
export default function Important() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        setLoading(true);
        const data = await fetchPdfs("important");
        setPdfs(data);
      } catch (err) {
        alert(err?.response?.data?.message || "Failed to load Important PDFs");
      } finally {
        setLoading(false);
      }
    };
      ...
      <div className="w-full max-w-7xl mx-auto">
        <div>
          ...
          <p className="mt-2 text-text/70">
            Most important questions & PDFs for quick revision.
          </p>
        </div>
        ...
        <div className="mt-8 p-5 rounded-3xl bg-card border border-border shadow-sm">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <input
              ...
              placeholder="Search important PDFs..."
