...
export default function College() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        setLoading(true);
        const data = await fetchPdfs("college");
        setPdfs(data);
      } catch (err) {
        alert(err?.response?.data?.message || "Failed to load college PDFs");
      } finally {
        setLoading(false);
      }
    };
      ...
      <div className="w-full max-w-7xl mx-auto">
        <div>
          ...
          <p className="mt-2 text-text/70">
            Semester notes, important PDFs and resources.
          </p>
        </div>
        ...
        <div className="mt-8 p-5 rounded-3xl bg-card border border-border shadow-sm">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <input
              ...
              placeholder="Search college PDFs..."
