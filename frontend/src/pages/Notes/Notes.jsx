...
export default function Notes() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        setLoading(true);
        const data = await fetchPdfs("notes");
        setPdfs(data);
      } catch (err) {
      ...
      <div className="w-full max-w-7xl mx-auto">
        ...
        <div>
          ...
          <p className="mt-2 text-text/70">
            Class 6 to Graduation Notes — Search & Download instantly.
          </p>
        </div>
        ...
        <div className="mt-8 p-5 rounded-3xl bg-card border border-border shadow-sm">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <input
              ...
              placeholder="Search notes by title / subject / class..."
        ...
        {loading ? (
          <div className="mt-10 p-10 rounded-3xl bg-card border border-border text-center">
            ...
            <p className="mt-4 font-bold text-text">Loading notes...</p>
          </div>
        ) : filtered.length === 0 ? (
          <div className="mt-10 p-10 rounded-3xl bg-card border border-border text-center">
            <p className="text-5xl">😕</p>
