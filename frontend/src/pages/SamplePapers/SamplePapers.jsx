...
export default function SamplePapers() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        setLoading(true);
        const data = await fetchPdfs("sample-papers");
        setPdfs(data);
      } catch (err) {
        alert(err?.response?.data?.message || "Failed to load Sample Papers");
      } finally {
        setLoading(false);
      }
    };
      ...
      <div className="w-full max-w-7xl mx-auto">
        <div>
          ...
          <p className="mt-2 text-text/70">
            Practice with Sample Papers for better performance.
          </p>
        </div>
        ...
        {loading ? (
          ...
          <div className="mt-10 p-10 rounded-3xl bg-card border border-border text-center">
            <p className="text-5xl">😕</p>
            <p className="mt-4 font-bold text-text">No sample papers found</p>
          </div>
        ) : (
